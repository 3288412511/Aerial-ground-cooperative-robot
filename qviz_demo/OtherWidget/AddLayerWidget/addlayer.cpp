  ...
  ui(new Ui::AddLayer)
{
   ...
   *********************/
  if (rclcpp::ok()) {
    auto topic_query_node = rclcpp::Node::make_shared("qviz_demo_topic_query");
    auto topic_map = topic_query_node->get_topic_names_and_types();
    for (const auto& topic : topic_map) {
      for (const auto& datatype : topic.second) {
        topicListAppendByTopicInfo(topic.first, datatype);
        treeWidget_ByTopic_addItemByTopic(ui->treeWidget_ByTopic, topic.first, datatype);
      }
    }
  }
 ...
 * @attention 本软件支持显示的消息类型没有RViz支持的多，所有支持显示的已在该函数的switch-case语句中罗列。当然笔者是根据需求开发的，用户可以继续使用RViz2接口在qrviz类中添加更多消息类型的显示支持！
 ...
 */
void AddLayer::topicListAppendByTopicInfo(const std::string& name, const std::string& datatype)
{
  switch (MsgTypeHash.value(QString::fromStdString(datatype))) {
    case AddLayer::laserscan:
      laserScanTopicList.append(QString::fromStdString(name));
    break;
    case AddLayer::pointcloud2:
      pointCloud2TopicList.append(QString::fromStdString(name));
    break;
    case AddLayer::map:
      mapTopicList.append(QString::fromStdString(name));
    break;
    case AddLayer::path:
      pathTopicList.append(QString::fromStdString(name));
    break;
    case AddLayer::pointstamped:
      pointStampedTopicList.append(QString::fromStdString(name));
    break;
    case AddLayer::marker:
      markerTopicList.append(QString::fromStdString(name));
    break;
    case AddLayer::markerarray:
      markerArrayTopicList.append(QString::fromStdString(name));
    break;
  }
}
