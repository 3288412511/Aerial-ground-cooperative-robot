...
/**
 * @brief 由nav_msgs::msg::Path类型的路径(path)变量初始化成员变量的构造函数(单个任务对应单条路径，所以创建任务对象时需要传递路径对象)
 * @param 行进模式动态数组（要求与path中的poses等长）
 * @param yaml文件的路径_fileStoragePath
 */
GenerateTask::GenerateTask(nav_msgs::msg::Path path, QVector<int> modeTypes, QString _fileStoragePath, QWidget *parent) :
  QDialog(parent),
...
void GenerateTask::saveTaskToFile(nav_msgs::msg::Path path, QVector<int> modeTypes, QString _fileStoragePath, QWidget *parent, const QString &title)
{
