...
public:
  ...
  void resizeEvent(QResizeEvent *event); // Overloaded function
	void showRos2InitFailureMessage();
...
public Q_SLOTS:
  ...
  void QProcessOpenMap();   //QProcess执行ROS2 nav2_map_server命令
  void GnomeOpenMap();      //终端执行ROS2 nav2_map_server命令(需ctrl + c手动销毁ros节点)
...
private:
  void ensureDualMapViewsInitialized();
  void ensureStrikeMissionViewInitialized();
  void updateDualMapViews();
  void initializeDualMapVideoOverlays();
  void updateDualMapVideoOverlayGeometry();
  QString resolveDemoVideoPath(const QString& stem) const;
  void playDualMapVideo(bool isCar);
  void stopDualMapVideo(bool isCar);
  void showMainMapPage();
  void showDualMapPage();
  void registerZoomablePreview(QLabel* label);
  void setZoomablePreview(QLabel* label, const QPixmap& pixmap);
  void refreshZoomablePreview(QLabel* label);
  void updateFpvOverlayGeometry();
  void startFpvStream();
  void stopFpvStream();
  void playLocalFpvVideo(const QString& filePath);
  QVariantMap defaultDualMapDisplayProperties(int layerCode, const QString& displayName, const QString& topicName) const;
  rviz_common::Display* createDualMapDisplay(qrviz* viz, int layerCode, const QVariantMap& properties);
  void applyDualMapDisplayProperties(qrviz* viz, rviz_common::Display* display, int layerCode, const QVariantMap& properties);
  QTreeWidgetItem* appendDualMapDisplayItem(QTreeWidget* treeWidget, rviz_common::Display* display, int layerCode, const QString& displayName, const QString& topicName);
  void addDisplayToDualMapView(qrviz* viz, QTreeWidget* treeWidget, const QString& dialogTitle);
  void removeSelectedDualMapDisplay(qrviz* viz, QTreeWidget* treeWidget);
  void refreshDualMapDisplayItem(QTreeWidgetItem* item);
  void updateDualMapDisplayDescription(QTreeWidget* treeWidget, QTextBrowser* descriptionBrowser);
  void editDualMapDisplay(qrviz* viz, QTreeWidgetItem* item);
  void initializeAerialPhotoPage();

	Ui::MainWindowDesign ui;
  bool isUsingStandardPoint;            //该bool值决定了用户使用RViz发布点位工具
  QHash<QTreeWidgetItem*, rviz_common::Display*> m_LayerHash;
  ...
  QNode qnode;
  qrviz* myviz = nullptr;
  qrviz* carMapViz = nullptr;
  qrviz* droneMapViz = nullptr;
  qrviz* strikeMissionViz = nullptr;
  rviz_common::Display* carGridDisplay = nullptr;
  rviz_common::Display* carMapDisplay = nullptr;
  rviz_common::Display* droneGridDisplay = nullptr;
  rviz_common::Display* droneMapDisplay = nullptr;
  rviz_common::Display* strikeMissionGridDisplay = nullptr;
  rviz_common::Display* strikeMissionMapDisplay = nullptr;
  rviz_common::Display* strikeMissionGoalDisplay = nullptr;
  QLabel* carMapVideoOverlay = nullptr;
  QLabel* droneMapVideoOverlay = nullptr;
  QTimer* carMapVideoTimer = nullptr;
  QTimer* droneMapVideoTimer = nullptr;
  cv::VideoCapture carMapVideoCapture;
  cv::VideoCapture droneMapVideoCapture;
  QString carMapCurrentVideoPath;
  QString droneMapCurrentVideoPath;
  QWebEngineView* aerialMapView = nullptr;
  QWidget* fpvOverlayPanel = nullptr;
  QStackedWidget* fpvVideoStack = nullptr;
  QLabel* fpvVideoPlaceholder = nullptr;
  QLineEdit* fpvRtspUrlEdit = nullptr;
  QPushButton* fpvStartButton = nullptr;
  QPushButton* fpvStopButton = nullptr;
  QLabel* fpvStatusLabel = nullptr;
  QTimer* fpvVideoTimer = nullptr;
  cv::VideoCapture fpvVideoCapture;
  QString fpvCurrentVideoPath;
  QHash<QLabel*, QPixmap> zoomablePreviewPixmaps;
  QHash<QLabel*, double> zoomablePreviewScales;
  ShellSelect* shellSelect = nullptr;
  AddLayer* addLayer = nullptr;
  //地图相关变量
